...
const startMongoose = async () => {
    ...
    try {
        ...
        console.log("Starting MessageCreatedListener...");
        new MessageCreatedListener(kafkaWrapper.consumer('chat-history-service-message-created')).listen().catch(err => {
            console.error("❌ Failed to start MessageCreatedListener:", err);
        });

        console.log("✅ All Kafka listeners initialization calls completed");

        app.listen(3000, () => {
            console.log("app listening on port 3000! chat-history service");
        });
