...
export class ProfileUpdatedListener extends Listener<ProfileUpdatedEvent> {
  ...
  readonly groupId = 'chat-service-profile-updated';
  ...
  async onMessage(data: ProfileUpdatedEvent['data'], payload: any) {
    ...
    if (existingUser) {
      ...
      console.warn(`[Profile Updated] User ${user} not found in chat service DB - profile data will be synced when user is created`);
    }
