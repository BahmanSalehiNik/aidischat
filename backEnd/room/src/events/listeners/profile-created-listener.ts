...
export class ProfileCreatedListener extends Listener<ProfileCreatedEvent> {
  ...
  readonly groupId = 'room-service-profile-created';
  ...
  async onMessage(data: ProfileCreatedEvent['data'], payload: any) {
    ...
    if (existingUser) {
      ...
      console.warn(`[Profile Created] User ${user} not found in room service DB - profile data will be synced when user is created`);
    }
