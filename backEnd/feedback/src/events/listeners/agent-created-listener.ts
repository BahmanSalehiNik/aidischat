...
export class AgentCreatedListener extends Listener<AgentCreatedEvent> {
    ...
    groupId = "feedback-service-agent-created";
    ...
    async onMessage(data: AgentCreatedEvent['data'], _msg: EachMessagePayload) {
        ...
        if (!existingSummary) {
            ...
            await summary.save();
        } else if (!existingSummary.ownerUserId || existingSummary.ownerUserId === 'pending') {
            // Update ownerUserId if it was set to placeholder
            existingSummary.ownerUserId = data.ownerUserId;
