...
export class AgentMatcher {
  ...
  async findRelevantAgents(
    ...
    const agents = await AgentProjection.find({
      isActive: true,
      isPublic: true,
    }).lean();
    ...
    const topMatches = matches.slice(0, ANALYSIS_CONFIG.MAX_INVITATIONS_PER_ANALYSIS);
   ...
   */
  private matchTopics(agent: any, topics: string[]): number {
    ...
    for (const topic of topics) {
      if (agentTags.some((tag: string) => tag.includes(topic.toLowerCase()) || topic.toLowerCase().includes(tag))) {
        matches++;
      }
    }
   ...
   */
  private matchDomain(agent: any, domain: string): number {
    const agentDomain = (agent.specialization || agent.profession || '').toLowerCase();
    const conversationDomain = domain.toLowerCase();

    if (!agentDomain) return 0;

    // Exact match
    if (agentDomain === conversationDomain) return 1;
   ...
   */
  private matchKeywords(agent: any, keywords: string[]): number {
   ...
   */
  private matchIntent(agent: any, intent: string): number {
    const agentPersonality = (agent.personality || []).map((p: string) => p.toLowerCase()).join(' ');
