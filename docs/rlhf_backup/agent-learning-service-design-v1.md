...
3. **State Projections**
   - Maintain per-agent materialized views: Summary, Feedback Aggregation, Relationship Strength, Behavior History.
   - Track rolling windows and decay functions for time-aware metrics.
4. **Policy Updating**
   - Periodically analyze projections to compute deltas for traits, action policies, and exploration values.
   - Enforce guardrails (min/max trait bounds, platform restrictions).
   - Emit `AgentLearningUpdatedEvent` for the agents service to persist.
5. **Dataset Generation**
   - Aggregate high-quality interactions and publish `TrainingDatasetReadyEvent`.
6. **Monitoring & Alerts**
